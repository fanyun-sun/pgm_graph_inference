...
class GGNN(nn.Module):
    ...
    def forward(self, J, b):
        ...
        for i in range(len(factors)):
            ...
            var2fac_edge_feat[u, i, :] = torch.FloatTensor([b[u], b[v], J[u,v].item(), J[v,u].item()])

            var2fac_edge_index.append([v, i])
            var2fac_edge_feat[v, i, :] = torch.FloatTensor([b[v], b[u], J[u,v].item(), J[v,u].item()])
        ...
        for i in range(len(factors)):
            ...
            fac2var_edge_feat[i, u, :] = torch.FloatTensor([b[u], b[v], J[u,v].item(), J[v,u].item()])
            fac2var_edge_index.append([i, v])
            fac2var_edge_feat[i, v, :] = torch.FloatTensor([b[v], b[u], J[u,v].item(), J[v,u].item()])
